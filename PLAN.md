...
**Recent progress:** 2026-05-02 — backend market summary routes (`/api/ihsg-chart`, `/api/market-summary`, `/api/sector-summary`) dimodularisasi ke `backend/routes/market_summary.py` dan parser sector snapshot ikut dipindah dengan guard tests struktur + runtime smoke via main app. News/corporate routes (`/api/news`, `/api/corporate-actions`, `/api/company-announcements`) juga dipisah ke `backend/routes/news.py` dengan guard tests struktur + runtime smoke via main app. Scanner SSE route (`/api/scan`) dan generator stream dipisah ke `backend/routes/scanner_stream.py` dengan guard tests struktur + runtime smoke via main app. Shared stock fallback helpers (`_ticker_base`, `_fallback_row_for_ticker`) dipindah ke `backend/routes/shared_stock_fallbacks.py` dan `stock_detail.py` direwire ke module itu dengan regression tests struktur + runtime smoke via main app. Shared SQLite datetime helper (`_sqlite_datetime_literal`) dipindah ke `backend/routes/shared_sqlite_helpers.py` dan main import direwire, diverifikasi dengan regression tests struktur + runtime smoke. Shared market helpers (`_top_mover_rows`, `_derived_broker_activity_rows`) dipindah ke `backend/routes/shared_market_helpers.py` dan `market.py` direwire, diverifikasi dengan regression tests struktur + runtime smoke. Latest follow-up: `market_summary.py` now reuses a shared sector-snapshot parser module, and regression coverage was added for that modularization. `stocks.py` now reuses shared ticker/company/search helpers, with regression coverage for the extraction and import wiring. `stock_detail.py` now reuses a shared OHLCV analysis helper, with regression coverage for the extraction and import wiring.
