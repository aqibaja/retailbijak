...
> **Execution status:** phase-1 first fold, phase-2 deep insight regrouping, phase-3 breadth+movers polish, phase-4 flow+catalyst refinement, phase-5 mobile-first ordering + responsive polish, phase-6 breadth endpoint optimization + denser Market Pulse refinement, phase-7 SQLite datetime binding fix untuk breadth production, phase-8 hardening shared latest-session OHLCV pipeline untuk movers, phase-9 snapshot consistency hardening untuk market stats + foreign flow, phase-10 dashboard market-intelligence sync fix + top-movers losers sorting production, phase-11 SPA cache-bust hardening untuk boot/router/dashboard shell, phase-12 full frontend import-chain cache-bust normalization lintas views, phase-13 cleanup artifact boot modules lama di repo/runtime, phase-14 guardrail regression test untuk single boot entry + versioned import chain + route coverage, phase-15 nav hash/router parity + repo-runtime core asset parity checker, phase-16 version-token consistency guardrail + script pre-deploy frontend parity checker, phase-17 deploy workflow auto pre-restart parity check + static deploy-doc coverage, phase-18 post-deploy smoke checker untuk homepage + endpoint inti, phase-19 runtime parity guardrail untuk semua routed views plus smoke checker marker SPA, phase-20 screener empty-state UX + deploy sync fix untuk mencegah nested `views/views` artifact, phase-21 stock detail AI-preview copy hardening supaya tidak lagi menampilkan placeholder/dev copy lintas ticker, phase-22 stock detail desktop snapshot panel hardening agar sisi kanan lebih hidup tanpa fake order book, phase-23 stock detail AI preview hardening dengan quick take, setup bias, valuation read, trade map, serta catalyst lens yang dinamis, phase-24 stock detail catalyst strip mini untuk news + IDX announcement dengan fallback source yang jujur, phase-25 catalyst strip relevansi ticker + external link hardening, phase-26 IDX announcement deep-link + catalyst ranking hardening, phase-27 catalyst strip relative-time polish, dan phase-28 FastAPI lifespan migration guard untuk startup/shutdown backend, phase-29 modularisasi route system health+scheduler backend, phase-30 modularisasi reference routes untuk timeframes+stocks dan import fallback hardening, phase-31 modularisasi stock search route + shared helper reuse hardening.
...
**Recent progress:** 2026-05-02 — backend market summary routes (`/api/ihsg-chart`, `/api/market-summary`, `/api/sector-summary`) dimodularisasi ke `backend/routes/market_summary.py` dan parser sector snapshot ikut dipindah dengan guard tests struktur + runtime smoke via main app. News/corporate routes (`/api/news`, `/api/corporate-actions`, `/api/company-announcements`) juga dipisah ke `backend/routes/news.py` dengan guard tests struktur + runtime smoke via main app. Scanner SSE route (`/api/scan`) dan generator stream dipisah ke `backend/routes/scanner_stream.py` dengan guard tests struktur + runtime smoke via main app. Shared stock fallback helpers (`_ticker_base`, `_fallback_row_for_ticker`) dipindah ke `backend/routes/shared_stock_fallbacks.py` dan `stock_detail.py` direwire ke module itu dengan regression tests struktur + runtime smoke via main app. Shared SQLite datetime helper (`_sqlite_datetime_literal`) dipindah ke `backend/routes/shared_sqlite_helpers.py` dan main import direwire, diverifikasi dengan regression tests struktur + runtime smoke. Shared market helpers (`_top_mover_rows`, `_derived_broker_activity_rows`) dipindah ke `backend/routes/shared_market_helpers.py` dan `market.py` direwire, diverifikasi dengan regression tests struktur + runtime smoke. Latest follow-up: `market_summary.py` now reuses a shared sector-snapshot parser module, with ticker/company/search helpers and stock-detail OHLCV analysis already modularized in sibling route modules. New follow-up on 2026-05-02: remaining market summary helpers (`_load_period_for_ihsg`, `_safe_pct`) were moved into `backend/routes/shared_market_summary_helpers.py`, structural guard tests were extended, `PLAN.md` was updated during the refactor, and runtime smoke/contract tests were re-verified successfully using the production-compatible venv at `/opt/swingaq/backend/venv` because the repo's default Python environment does not have the `ta` package installed. Final follow-up for this pass: the route-local duplicate parser definition was removed from `backend/routes/market_summary.py`, parser tests now import the shared helper directly, modularization guards now assert that all three helper definitions stay out of the route module, and the runtime + contract subset remains green. New follow-up after that: duplicate stock-search helpers (`_search_rank`, `_search_bucket`) were removed from `backend/routes/stocks.py`, guard coverage now asserts those helpers stay only in `backend/routes/shared_stocks_helpers.py`, and the stock-search route modularization/runtime subset remains green under the production-compatible venv. Latest follow-up in this pass: shared market helpers were decoupled from `backend/routes/stocks.py` by importing `_company_name` directly from `backend/routes/shared_stocks_helpers.py`, a dedicated `test_shared_market_helpers_modularization.py` guard was added, and the market modularization/runtime subset remains green. Latest follow-up in this pass: stock detail ticker normalization now reuses shared fallback helper `_ticker_with_suffix` from `backend/routes/shared_stock_fallbacks.py` instead of repeating `.JK` suffix logic across `stock_detail.py`, chart-data serialization now falls back to the dataframe row index when the `date` column is absent, and the updated structural/runtime subset passed again (`7 passed`) under `/opt/swingaq/backend/venv`. Latest follow-up in this pass: `backend/routes/shared_stocks_helpers.py` now reuses `_ticker_base` from `backend/routes/shared_stock_fallbacks.py` so ticker normalization is no longer duplicated across sibling shared modules, the stock helper modularization guards were tightened accordingly, and the stock search modularization/runtime subset passed again (`8 passed`) under `/opt/swingaq/backend/venv`. Latest follow-up in this pass: `backend/routes/shared_market_helpers.py` now reuses `_display_ticker` from `backend/routes/shared_stocks_helpers.py` instead of importing `get_ticker_display` from the base `stocks.py` module, further reducing shared-helper coupling to non-route modules, and the market modularization/runtime subset passed again (`5 passed`) under `/opt/swingaq/backend/venv`. Latest follow-up in this pass: `backend/routes/scanner_stream.py` now reuses `_display_ticker` from `backend/routes/shared_stocks_helpers.py` for emitted scan result labels instead of importing `get_ticker_display` from the base `stocks.py` module, and the scanner stream modularization/runtime subset passed again (`3 passed`) under `/opt/swingaq/backend/venv`. Latest follow-up in this pass: `backend/routes/shared_stocks_helpers.py` no longer imports `get_ticker_display` from the base `stocks.py` module and now falls back directly to the normalized base ticker for unknown company labels, which keeps sibling shared helpers fully self-contained on top of `_ticker_base`; the broader shared-stock/search/scanner/market modularization subsets stayed green (`16 passed`) under `/opt/swingaq/backend/venv`. Latest follow-up in this pass: the stock-detail signals route now reuses `_ticker_base` directly for `Signal.ticker` filtering instead of trimming the `.JK` suffix inline, extending the shared ticker-normalization cleanup into the last remaining signal-query path; the related stock-detail modularization/runtime subset passed again (`5 passed`) under `/opt/swingaq/backend/venv`. Latest follow-up after that: stock-detail signal row serialization was moved into `backend/routes/shared_stock_detail_helpers.py` via `_serialize_signal_rows`, the `/signals` endpoint was added to the stock-detail runtime smoke coverage, and the related helper/runtime subset passed under `/opt/swingaq/backend/venv` (`4 passed`).
