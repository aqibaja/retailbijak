...
export async function renderMarket(root) {
  ...
  document.getElementById('corporate-actions').innerHTML = card('Aksi Korporasi', 'Pencatatan, dividen, suspensi, dan agenda emiten lain dari IDX', `<div class="market-section-summary">${safeRows(actionsData).length ? `${safeRows(actionsData).length} event terbaru untuk dipantau di sesi ini.` : 'Belum ada event korporasi yang menonjol untuk sesi ini.'}</div><div class="market-list-stack">${safeRows(actionsData).slice(0, 3).map(actionRow).join('') || emptyState('Belum ada aksi korporasi yang relevan.', 'Pantau lagi setelah sinkronisasi IDX berikutnya untuk event korporasi terbaru.')}</div>`).replace('class="market-card"', 'class="market-card market-card-feed"');
  document.getElementById('foreign-flows').innerHTML = card('Arus Investor Asing', 'Ringkasan perdagangan IDX untuk partisipasi investor asing', `<div class="market-section-summary">${safeRows(foreignData).length ? `${safeRows(foreignData).filter((row) => Number(row.net_value ?? 0) >= 0).length} saham net buy asing, ${safeRows(foreignData).filter((row) => Number(row.net_value ?? 0) < 0).length} saham net sell asing.` : 'Belum ada snapshot foreign flow yang valid untuk sesi ini.'}</div><div class="market-list-stack">${safeRows(foreignData).slice(0, 3).map(flowRow).join('') || emptyState('Belum ada foreign flow yang tervalidasi.', 'Feed foreign participation belum mengirim snapshot yang cukup untuk sesi ini.')}</div>`, 'var(--text-up)').replace('class="market-card"', 'class="market-card market-card-feed"');
