...
function renderLevelOverlay(candles, tech){
  const overlay=document.getElementById('level-overlay'); if(!overlay) return; const levels=getLevels(candles, tech);
  const risk = Math.max(levels.entry - levels.stop, 1), reward = Math.max(levels.target - levels.entry, 1);
  const levelMin = levels.stop - risk * .25, levelMax = levels.target + reward * .18;
  const line=(label,price,color)=>{ if(!price || levelMax===levelMin) return ''; const raw=100-((price-levelMin)/(levelMax-levelMin)*100); const pct=Math.max(6,Math.min(94,raw)); return `<div class="level-line" data-level="${label}" style="top:${pct}%;border-color:${color}"></div>`};
