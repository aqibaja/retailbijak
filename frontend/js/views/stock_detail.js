...
export async function renderStockDetail(root, ticker) {
  ...
  document.querySelectorAll('.stock-range').forEach(btn => btn.addEventListener('click', () => {
    ...
    try { renderStockChart(candles, Number(btn.dataset.limit || 30)); renderLevelOverlay(candles, technical); renderLevelLegend(candles, technical); } catch (e) { renderFallbackSvgChart(candles.slice(-Number(btn.dataset.limit || 30))); renderLevelOverlay(candles, technical); renderLevelLegend(candles, technical); }
  }));
  ...
  renderLevelLegend(candles, technical);
}
...
function getLevels(candles, tech){
  ...
  const entry = last.close; const rawStop = isValidLevel(sr.support_20d) ? Number(sr.support_20d) : entry - (atr || entry*.06); const tightStop = entry - Math.min(atr ? atr*.5 : entry*.06, entry*.06); const stop = Math.max(rawStop, tightStop); const target = isValidLevel(sr.resistance_20d) ? Number(sr.resistance_20d) : entry + (atr || entry*.08);
...
function renderLevelOverlay(candles, tech){
  ...
  const line=(label,price,color)=>{ if(!price || max===min) return ''; const raw=100-((price-min)/(max-min)*100); const pct=Math.max(6,Math.min(94,raw)); return `<div class="level-line" data-level="${label}" style="top:${pct}%;border-color:${color}"></div>`};
  overlay.innerHTML = line('STOP', levels.stop, '#fb7185') + line('ENTRY', levels.entry, '#34d399') + line('TARGET', levels.target, '#fbbf24');
}
function renderLevelLegend(candles, tech){
  const el=document.getElementById('level-legend'); if(!el) return; const levels=getLevels(candles, tech);
  const chips=[['STOP', levels.stop, '#fb7185'], ['ENTRY', levels.entry, '#34d399'], ['TARGET', levels.target, '#fbbf24']];
  el.innerHTML = chips.map(([label, price, color]) => `<span class="level-chip"><i class="level-dot" style="background:${color}"></i>${label} <strong class="mono">${money(price)}</strong></span>`).join('');
}
