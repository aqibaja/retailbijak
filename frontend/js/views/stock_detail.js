  ...
  root.innerHTML = `
    <section class="stock-detail-pro stock-detail-compact stagger-reveal">
      <style>
        ...
        .technical-grid,.stats-grid,.insight-grid,.right-uniform-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(128px,1fr));gap:7px}.technical-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.metric-group{margin-top:8px}.metric-group-title{font-size:10px;text-transform:uppercase;color:var(--text-muted);font-weight:900;letter-spacing:.08em;margin:6px 0 5px;display:flex;align-items:center;gap:6px}.decision-panel{display:grid;grid-template-columns:1fr;gap:10px}.below-chart-fill{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:10px}.below-chart-fill .stat-tile{min-height:72px}.ai-chat-placeholder{flex:1;margin-top:10px;min-height:220px;display:flex;flex-direction:column;border:1px dashed rgba(34,211,238,.25);background:linear-gradient(135deg,rgba(14,165,233,.08),rgba(15,23,42,.38));border-radius:16px;padding:14px}.ai-chat-box{display:flex;gap:10px;align-items:center;justify-content:space-between}.ai-chat-input{margin-top:auto;height:42px;border-radius:12px;border:1px solid rgba(255,255,255,.08);background:rgba(2,6,23,.55);display:flex;align-items:center;padding:0 12px;color:var(--text-muted);font-size:13px}.decision-hero{padding:10px 12px;border-radius:16px;border:1px solid rgba(148,163,184,.20);background:linear-gradient(135deg,rgba(15,23,42,.84),rgba(2,6,23,.48))}.action-bar{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.chart-shell{position:relative}.level-overlay{position:absolute;inset:0;pointer-events:none}.level-line{position:absolute;left:0;right:0;border-top:1px dashed rgba(255,255,255,.35)}.level-line span{position:absolute;right:10px;top:-11px;font-size:10px;padding:2px 6px;border-radius:999px;background:#0f172a;border:1px solid rgba(255,255,255,.12)}.stat-tile{min-height:68px;padding:8px 9px;border-radius:14px;background:rgba(255,255,255,.035);border:1px solid rgba(255,255,255,.06)}.stat-tile span{display:block;font-size:10px;text-transform:uppercase;color:var(--text-dim);font-weight:800;letter-spacing:.06em}.stat-tile strong{display:block;margin-top:3px;font-size:14px;line-height:1.15}.stat-tile small{display:inline-flex;margin-top:2px;font-size:10px;color:var(--text-muted)}
...
function renderLevelOverlay(candles, tech){
  ...
  const line=(label,price,color,slot=0)=>{ if(!price || max===min) return ''; const raw=100-((price-min)/(max-min)*100); const pct=Math.max(6,Math.min(94,raw)); const right = 10 + slot*116; return `<div class="level-line" style="top:${pct}%;border-color:${color}"><span class="balanced-level-label" style="right:${right}px;color:${color}">${label} ${money(price)}</span></div>`};
  overlay.innerHTML = line('STOP', levels.stop, '#fb7185', 0) + line('ENTRY', levels.entry, '#34d399', 1) + line('TARGET', levels.target, '#fbbf24', 2);
}
