...
export async function renderStockDetail(root, ticker) {
    ...
    <section class="stock-detail-pro stock-detail-compact stagger-reveal">
      ...
      <div class="stock-layout">
        <div class="panel stock-chart-card">
          ...
          <div class="ai-chat-placeholder"><div class="ai-chat-box"><div><div class="text-xs text-dim uppercase strong">AI Assistant</div><div class="text-sm text-main strong mt-1">Ask AI about this stock</div><div class="text-xs text-muted mt-1">UI placeholder — nanti terhubung ke AI analis RetailBijak.</div></div><span class="signal-pill pill-good">Coming Soon</span></div><div class="ai-chat-input">Tanya: risk, entry, news, atau alasan sinyal...</div></div>
        </div>
        <div class="stock-side compact-right-scroll flex-col gap-2">
          <div class="panel"><div class="flex justify-between items-start gap-3"><div><h3 class="panel-title mb-2">Technical Summary</h3><div id="technical-summary" class="intel-item">Loading technical...</div></div><div id="signal-card" class="signal-card"><span>Signal</span><strong>—</strong><small>Confidence —</small></div></div><div id="technical-panel" class="technical-grid mt-3"></div></div>
          <div class="panel"><h3 class="panel-title mb-3">Key Statistics</h3><div id="fundamental-panel" class="stats-grid compact-grid-3"></div></div>
          <div class="panel"><h3 class="panel-title mb-3">Analysis Snapshot</h3><div id="analysis-panel" class="insight-grid compact-grid-3"></div></div>
  ...
  document.querySelectorAll('.stock-range').forEach(btn => btn.addEventListener('click', () => {
    ...
    try { renderStockChart(candles, Number(btn.dataset.limit || 30)); renderLevelOverlay(candles, technical); } catch (e) { renderFallbackSvgChart(candles.slice(-Number(btn.dataset.limit || 30))); renderLevelOverlay(candles, technical); }
  }));
...
function renderLevelOverlay(candles, tech){
  ...
  const line=(label,price,color)=>{ if(!price || max===min) return ''; const pct=100-((price-min)/(max-min)*100); const right = label === 'ENTRY' ? '124px' : '10px'; return `<div class="level-line" style="top:${Math.max(4,Math.min(96,pct))}%;border-color:${color}"><span style="right:${right};color:${color}">${label} ${money(price)}</span></div>`};
