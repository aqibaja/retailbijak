      ...
      <section class="market-overview-page">
        ...
        <div class="market-section-group">
          <div id="news-featured" class="market-section-group-grid" style="grid-template-columns:1.4fr 1fr;gap:18px">
            <div class="market-card" id="news-featured-main" style="min-height:280px;padding:0;overflow:hidden;border-radius:20px">
              <div class="skeleton skeleton-card" style="height:280px;border-radius:20px"></div>
            </div>
    ...
    try {
        ...
        if (!items.length) {
          document.querySelectorAll('[id^=news-]').forEach(el => {
            if (el.id !== 'news-count') el.innerHTML = '<div class="dashboard-widget-state" style="grid-column:1/-1"><strong class="dashboard-widget-state-title">Belum ada berita</strong><span class="dashboard-widget-state-note">Berita akan muncul setelah scheduler berjalan.</span></div>';
          });
          return;
        }
        ...
        if (stream) stream.innerHTML = '<div class="dashboard-widget-state" style="grid-column:1/-1"><strong class="dashboard-widget-state-title">Gagal memuat berita</strong><span class="dashboard-widget-state-note">Coba refresh halaman.</span></div>';
    }
}
